...
int main(int argc, char **argv) {
  ...
  if (argc < 12) {
    ...
    /* Ensure central bright star (15-18 mag) for StarFinder*/
    if (stars_mag_bright == 0.0 ){
        nstar=1;
    } else {
    ...
    if (xbox_star_pos_c == -1 ){
        ...
        //printf ("Central star mag for preseleted ROI: %f\n",starmag[0]) ;
    ...
    if (stars_mag_bright == 0.0 ){
        ...
        ys[0] = ny/2.;
        starmag[0] = 15 + ran2(&ldummy)*3;
        //printf ("Central star mag for StarFinder: %f\n",starmag[0]) ;
        //printf ("Pos x: %f\n",xs[0]) ;
        //printf ("Pos y: %f\n",ys[0]) ;
        fl[0]=pow(10.,(-.4*( starmag[0] - zeropoint)))*exptime ;
        }
    ...
    if (iseq != 0){
      sprintf(str_tmp,"%s/tmp/ldummy.temp",working_directory);
    ...
    sprintf(buf,"%s/tmp/decam-guide_ccd%d.fits",working_directory,iCCD);
    ...
    /* Write stars to file */
    if (iseq == 0){
        printf("First iteration, save stars.\n");
        sprintf(str_tmp,"%s/stars_ccd%d.dat",working_directory,iCCD);
        if (!(fd2 = fopen(str_tmp,"w"))) {
            printf ("Failed to create stars file");
            error(0);
        } else {
            fprintf(fd2,"#Num   Xpos     Ypos       mag       flux\n");
            for (k=0;k<nstar;k++) {
                fprintf(fd2,"%4d %8.3f %8.3f %8.2f %12.3e\n",k,xs[k],ys[k],starmag[k],fl[k]);
            }
            fclose(fd2);
        }
    }
...
#define MAXTAGS 300
  ...
  if (fd = fopen(fname, "r")) {
    sprintf(buf, "%s/tmp/%s",working_directory, "sim-usedvalues.params");
